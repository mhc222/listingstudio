...
**Work items**
1. **Finalize returns immediately.** The route keeps auth, ownership, and the atomic claim (`reserved|failed → finalizing`) exactly as now, then returns `202 { itemId, status: "finalizing" }` and runs the existing materialize → RPC → cleanup → batch-refresh body inside Next 15.5 `after()` from `next/server` (stable; runs post-response within `maxDuration = 120` on Vercel). Extract that body into `lib/intake-finalize.ts` `finalizeUploadItem(itemId, userId, admin)` so the route and the cron share one function. Idempotency stays: a second call on an item already `finalizing` returns 202 without re-running unless `updated_at` is older than `FINALIZE_STALE_MS` (3 min), in which case it re-claims and re-runs (materialize and the RPC are already idempotent). Failures inside `after()` set `status='failed'`, `error=<message>` as today so Retry still appears.
2. **Cron recovery.** `app/api/cron/reconcile` additionally sweeps `upload_items` with `status='finalizing'` and `updated_at < now() - FINALIZE_STALE_MS` (cap 20 per run) through the same `finalizeUploadItem`; also sweeps `reserved` items older than 24 h to `failed` with error "upload never completed" so they stop counting as in flight. Log counts in the existing results object. No schema change: statuses `reserved|finalizing|complete|failed|canceled` and `updated_at` already exist (migration 0009).
3. **Client stops waiting.** In `app/listings/[id]/upload-queue.tsx`: the transfer slot is released when the PUT completes; `finalizeItem` is fire-and-forget (it just records the 202 and moves the item to `finalizing`). Items in `finalizing` render in the photo grid as **placeholder tiles** using a local `URL.createObjectURL(file)` preview (revoked on unmount/replace) with a subtle "Processing" badge, in the position the photo will take. When the photo row appears (Phase 55 realtime/poll refresh, unchanged cadence), the placeholder is replaced by the real thumb. Items with no local file (page reloaded mid-batch, hydrated from `GET /api/uploads`) render a neutral processing tile. Leaving the page is safe and the UI must say so once per batch ("Uploads continue in the background") instead of a blocking spinner. Keep the existing per-item Retry/Cancel affordances.
4. **Drop the redundant authorize.** `prepare` already returns `signedUrl`+`token` per item; `startTransfer` must use them for the first PUT and call `/authorize` only on retry, resume after reload, or token expiry. Result: 2 calls per photo (PUT, finalize) plus one prepare per batch.
5. **Tests.** `scripts/test-background-uploads.mjs` (`npm run test:background-uploads`, ≥20 assertions): route returns before materialize (mock admin client with a slow materialize), 202 shape, stale re-claim vs fresh no-op, failure inside the deferred body marks `failed` with error, cron sweep selects only stale `finalizing`/old `reserved` rows and caps at 20, client scheduler releases the slot on PUT completion and does not await finalize (contract test over the source like Phase 55's), placeholder object URL revoked. All Phase 44–56 suites still pass.

**Not in scope:** ordering edits on photos whose row does not exist yet (needs photo rows created at prepare time; Phase 58 candidate), making materialize itself faster, resumable/chunked uploads, changing `MAX_CONCURRENT_UPLOADS`, any migration, `next/image`, pricing/metering.

**Definition of done:** `npm run test:background-uploads` ≥20 assertions + all Phase 44–56 suites, `npx tsc --noEmit`, `npm run lint`, `npm run build` all pass; no migration; PROGRESS/DECISIONS updated. **Manual test (Matt):** drop 10 photos on a listing; all 10 placeholder tiles appear within a second; devtools Network shows each finalize responding in well under 1 s and exactly 2 requests per photo after prepare; real thumbs replace placeholders within ~10 s without any click; repeat with 5 photos and close the tab right after the drop finishes transferring, reopen after 20 s: all 5 photos present, none stuck. Force a stuck row (set one item to `finalizing` with `updated_at` 10 min old) and hit the reconcile cron: it completes or fails the item, never leaves it.
