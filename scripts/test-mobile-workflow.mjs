...
contains(listingPage, 'className="inline-flex min-h-10', "listing back navigation is below 40px")
...
const shootOrganization = read("app/listings/[id]/shoot-organization.tsx")
contains(shootOrganization, "flex min-h-10 min-w-0 cursor-pointer", "shoot selection labels are below 40px")
...
contains(proofing, "The selected image could not load", "proofing does not identify stale or missing image links")
contains(proofing, "imageFailed || (selectedVersion && !selectedVersion.url)", "proofing silently falls back when a selected result has no secure URL")
